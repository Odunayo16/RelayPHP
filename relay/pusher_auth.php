<?php
// This file handles Pusher authentication for private/presence channels. Specifically it deals with two types of channels:
// 1. We accept presence channel names of the format "presence-Princeton-University-online", where Princeton-University can be replaced by any school
// 	  In these types of channels, any user from Princeton University can subscribe. We use it to determine online presence.
// 2. We accept private channel names of the format "private-inbox-5827", where this channel represents the inbox for user 5827. 
//	  Within this inbox, users can receive "anything," but for now, we will only allow them to receive private messages (using pusher events).
// 3. We accept private channel names of the format "private-random-chat-inbox-5827", where this channel represents the random chat inbox for user 5827
//	  Within this inbox, users can receive random chat events/messages

header('Access-Control-Allow-Origin: http://friendsy.thesis.com');
